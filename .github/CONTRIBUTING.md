# Contributing<br>to `collaborative-law`

> [![PRs Welcome][makeapullrequest-image]][makeapullrequest-url]
>
> Welcome to `collaborative-law`. You're among people eager to promote recommended community standards that encourage open source consumption and contributions with comprehensive `README`, `CODE_OF_CONDUCT`, `CONTRIBUTING`, and `LICENSE` documents. If you are curious, you're already a member!
...
* **Collaboration starts with *Issues*. Changes happen through *Pull Requests*.**
    ...
    View `collaborative-law's` collaboration and contribution flowcharts:
...
* ### 1.1. Create Issues for feature requests and defects.
    ...
    > ⌦ `collaborative-law` follows an issue-driven product delivery model.
...
* ### 2.1. **Rules**
  ...
  `collaborative-law` manages contributions with the  [feature-branch-workflow](https://www.atlassian.com/git/tutorials/comparing-workflows#feature-branch-workflow).

* #### 2.1.1. Makes changes in a topic branch.

  _Why:_
  > ⌦ Use an isolated topic branch for parallel product development. Topic branches allow you to submit multiple pull requests without confusion. You can iterate without polluting the master branch with potentially unstable, unfinished code. The `collaborative-law` team uses:
  >
...
* #### 2.1.3. Branch out from `master`.
    ...
    _Why:_
    > ⌦ `collaborative-law` follows the feature-branch-workflow.
...
* ### 3.1. Use the Standard JS Style.
  ...
  `collaborative-law` follows the [Standard JS Style][standard-js-url].
...
* ### 3.2. Use ESLint to analyze source code.
  ...
  View [`collaborative-law's` ESLint rules][eslint-rules-table-url] and their enforcement.
...
`collaborative-law` requires the following tech stack to either run, build, test, or deploy:
...
[cla-url]: https://www.clahub.com/agreements/commonality/collaborative-law
...
[stackshare-url]: https://stackshare.io/commonality/collaborative-law
